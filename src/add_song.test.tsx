...
describe('<AddSong />', () => {
  ...
  it('selects spotify link correctly', () => {
    const valid_spotify_url:string = "https://open.spotify.com/track/2374M0fQpWi3dLnB54qaLX?si=qh4n4skbTFK1nUyEQobiIQ";
    const wrapper = shallow(<AddSong />) 
    expect(wrapper.instance().determineURLType(valid_spotify_url)).toBe(URLTYPE.SPOTIFY)
  });
  it('selects soundcloud link correctly', () => {
    const valid_soundcloud_url:string = "https://soundcloud.com/lifeofdesiigner/desiigner-panda" 
    const wrapper = shallow(<AddSong />) 
    expect(wrapper.instance().determineURLType(valid_soundcloud_url)).toBe(URLTYPE.SOUNDCLOUD)
  });
  it('selects youtube link correctly', () => {
    const valid_youtube_url:string = "https://www.youtube.com/watch?v=Jg5wkZ-dJXA";
    const wrapper = shallow(<AddSong />) 
    expect(wrapper.instance().determineURLType(valid_youtube_url)).toBe(URLTYPE.YOUTUBE)
  }),
  it('selects error on invalid link', () => {
    const invalid_domain_url:string = "invalid.url";
    const wrapper = shallow(<AddSong />) 
    expect(wrapper.instance().determineURLType(invalid_domain_url)).toBe(URLTYPE.ERROR)
  })
